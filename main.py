...
class Pipeline:
    ...
    def run_classification(self, mode='both'):
        ...
        # Charger encodeur pré-entraîné
        print("\nChargement de l'encodeur pré-entraîné...")
        checkpoint_path = (self.config.paths.checkpoints_dir / 
                          'cross_modal' / 'best_model.pt')
        ...
        if mode in ['linear_probe', 'both']:
            ...
            trainer_probe = ClassificationTrainer(
                classifier_probe, self.config, self.device, mode='linear_probe'
            )
            trainer_probe.fit(clf_loaders['train'], clf_loaders['val'])
            
            # Évaluation
            evaluator_probe = Evaluator(classifier_probe, self.config, self.device)
        ...
        if mode in ['finetune', 'both']:
            ...
            trainer_ft = ClassificationTrainer(
                classifier_ft, self.config, self.device, mode='finetune'
            )
            trainer_ft.fit(clf_loaders['train'], clf_loaders['val'])
            
            # Évaluation
            evaluator_ft = Evaluator(classifier_ft, self.config, self.device)
        ...
        if mode == 'both':
            ...
            # Sauvegarder
            comparison.to_csv(
                self.config.paths.results_dir / 'classification_comparison.csv'
            )
    ...
    def run_evaluation(self):
        ...
        # Sauvegarder
        fs_results.to_csv(
            self.config.paths.results_dir / 'fewshot_results_raw.csv',
            index=False
        )
        fs_agg.to_csv(
            self.config.paths.results_dir / 'fewshot_results_agg.csv',
            index=False
        )
    ...
    def generate_final_report(self):
        ...
        """
        import json
        from utils import create_results_summary
        ...
        try:
            classification_results = pd.read_csv(
                results_dir / 'classification_comparison.csv',
                index_col=0
            )
        ...
        try:
            fewshot_results = pd.read_csv(
                results_dir / 'fewshot_results_agg.csv'
            )
